...
//		codec_wr_control(0x48c, 0x7ff, 0, 0x0);
	...
	if (agc_used){
		...
		codec_wr_control(0x210, 0x3, 6, 0x3);
		codec_wr_control(0x214, 0x3, 6, 0x3);
		codec_wr_control(0x408, 0x7, 12, 0x2);
		codec_wr_control(0x40c, 0x7, 12, 0x2);
		codec_wr_control(0x4d0, 0x3, 6, 0x3);
		codec_wr_control(0x44c, 0xff, 0, 0xbf);
	}
	/*mute headphone pa*/
...
//	codec_wr_control(SUNXI_AIF1_MXR_SRC, 0xf, AIF1_AD0R_MXR_SRC, 0x8);
	...
	//printk("=====[%s(%d)]:headset_plug_state = %d.\n", __FUNCTION__, __LINE__, headset_plug_state);	
	if (1 == headset_plug_state){
		...
		codec_wr_prcm_control(MIC2G_LINEEN_CTRL, 0x7, MIC2BOOST, cap_vol);
	} else {
